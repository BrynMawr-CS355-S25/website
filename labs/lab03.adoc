...
* Use the command: `kill -STOP <pid>` to stop the process. 
* Use the command: `kill -CONT <pid>` to continue the process. 
* Use the command: `kill -TERM <pid>` to terminate the process. 
...
0. What happens if the parent and child both belong to the same process group?
1. What happens when SIGCONT is sent to a process that is already running?
2. What happens when SIGSTP is sent to a process already stopped?
3. What happens when you send the signal SIGTTOU?
4. What happens when you send he signal SIGALRM?
